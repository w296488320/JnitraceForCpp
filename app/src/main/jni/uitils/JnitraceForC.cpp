...
static const std::list<string> filterSoList ;
...
static inline void write(const std::string &msg,bool isApart) {
    if (isSave) {
        std::lock_guard<std::mutex> lock(m_lock);
        os << msg;
    }
...
JNI_HOOK_DEF(jboolean, CallStaticBooleanMethodV, JNIEnv *env, jclass obj, jmethodID jmethodId,
    ...
    for(const auto &soname : filterSoList) {
        if (strstr(info.dli_fname, soname.c_str())) {
			...
			//LOG(INFO) << "invoke method result Boolean : " << (ret == JNI_TRUE?"true":"false");
			//os << "invoke method result Boolean :  " <<(ret == JNI_TRUE?"true":"false") << "\n";;
			write(string("result Boolean : ").append((ret == JNI_TRUE ? "true" : "false")).append(
					"\n"));
			return ret;
		}
    }
...
void Jnitrace::getJObjectInfoInternal(JNIEnv *env,
    ...
    if (ret != nullptr) {
        if (isPrintClassinfo) {
            //LOGI("%s ->  %s   -> %s ", message.c_str(), classInfo, ret)
            //if(isSave){
            //os<<message<<" "<<classInfo<<"  "<< ret<<"\n";
            string basicString = message.append("  ").append(classInfo).append("  ").append(
                    ret).append("\n");
            write(basicString);
            //}
        } else {
            //LOGI("%s ->   -> %s ", message.c_str(), ret)
            //if(isSave){
            //os<<message<<"  "<< ret<<"\n";
...
//        LOGI("->   %s   ", classInfo)
        ...
        if (strcmp(classInfo, "boolean") == 0 || strcmp(classInfo, "java.lang.Boolean") == 0) {
            ...
            if (strcmp(classInfo, "[B") == 0) {
                ...
                jmethodID methodid = env->GetStaticMethodID(ArrayClazz, "toString",
                                                            "([Z)Ljava/lang/String;");
                argJstr = static_cast<jstring>(env->CallStaticObjectMethod(ArrayClazz, methodid,
                                                                           arg));
            }

            //上面的逻辑主要是为了处理argJstr的赋值
            const char *ret = env->GetStringUTFChars(argJstr, nullptr);
            if (ret != nullptr && ret != NULL) {
                //LOGI("%s ->  %s  UTF-8 encode -> %s ", argsInfo, classInfo, ret)
                //os<<argsInfo<<" "<<classInfo<<" UTF-8 encode "<< (ret == NULL?"":ret)<<"\n";
                write(string(argsInfo).append(" class info ->  ")
                              .append(classInfo).append(" ").append(
                                (ret == NULL ? "" : ret)).append("\n"));
            }
            continue;
        } else {
 ...
 * get obj to string
 */
char *Jnitrace::getJObjectToString(JNIEnv *env, jobject obj) {
    jmethodID method_id_toString =
            env->GetMethodID(env->FindClass("java/lang/Object"),
                             "toString",
                             "()Ljava/lang/String;");
    auto pJstring = static_cast<jstring>(env->CallObjectMethod(obj, method_id_toString));
    return const_cast<char *>(env->GetStringUTFChars(pJstring, nullptr));
}
...
JNITRACE_HOOK_DEF(jstring, NewStringUTF, JNIEnv *env, const char *bytes) {
    ...
    for(const auto &soname : filterSoList) {
        ...
        //os << "NewStringUTF : " << bytes << "\n";
        write(string("NewStringUTF : ").append(bytes).append("\n"));
...
JNITRACE_HOOK_DEF(const char*, GetStringUTFChars, JNIEnv *env, jstring argstring, jboolean *isCopy) {
    DL_INFO
    for(const auto &soname : filterSoList) {
        if (strstr(info.dli_fname, soname.c_str())) {
        const char *chars = orig_GetStringUTFChars(env, argstring, isCopy);
        //LOG(INFO) << "GetStringUTFChars : " << chars;
        //os << "GetStringUTFChars : " << chars << "\n";
...
JNITRACE_HOOK_DEF(jclass, FindClass, JNIEnv *env, const char *name) {
    ...
    for(const auto &soname : filterSoList) {
        ...
        //os << "find class : " << name << "\n";
        write(string("find class : ").append(name).append("\n"));
...
JNITRACE_HOOK_DEF(jobject, NewObjectV, JNIEnv *env, jclass clazz, jmethodID jmethodId, va_list args) {
    DL_INFO
    for(const auto &soname : filterSoList) {
        if (strstr(info.dli_fname, soname.c_str())) {
...
JNITRACE_HOOK_DEF(jobject, ToReflectedMethod, JNIEnv *env, jclass clazz, jmethodID jmethodId,
         jboolean isStatic) {
    DL_INFO
    for(const auto &soname : filterSoList) {
        if (strstr(info.dli_fname, soname.c_str())) {
...
JNITRACE_HOOK_DEF(jobject, FromReflectedMethod, JNIEnv *env, jobject method) {
    ...
    for(const auto &soname : filterSoList) {
        if (strstr(info.dli_fname, soname.c_str())) {
        Jnitrace::getJObjectInfoInternal(env, method, "FromReflectedMethod arg ", false, nullptr);
...
JNITRACE_HOOK_DEF(jfieldID, GetFieldID, JNIEnv *env, jclass clazz, const char *name, const char *sig) {
    DL_INFO
    for(const auto &soname : filterSoList) {
        if (strstr(info.dli_fname, soname.c_str())) {
        jmethodID method_id_toString = env->GetMethodID(
        ...
        write(string("get field info  : ").append(toString)
...
JNITRACE_HOOK_DEF(jfieldID, GetStaticFieldID, JNIEnv *env, jclass clazz, const char *name, const char *sig) {
    DL_INFO
    for(const auto &soname : filterSoList) {
        if (strstr(info.dli_fname, soname.c_str())) {
        jmethodID method_id_toString = env->GetMethodID(
        ...
        write(string("get static field info  : ").append(toString)
                      .append(" ").append(name).append(" ").append(sig).append("\n"));
...
JNITRACE_HOOK_DEF(jobject, GetObjectField, JNIEnv *env, jobject argObj, jfieldID jfieldId) {
    DL_INFO
    for(const auto &soname : filterSoList) {
        if (strstr(info.dli_fname, soname.c_str())) {
        ...
        if (ret != nullptr) {
            Jnitrace::getJObjectInfoInternal(env, ret, "GetObjectField result object :", true,
                                             nullptr);
        }
        return ret;
    }
...
JNITRACE_HOOK_DEF(jboolean, GetBooleanField, JNIEnv *env, jobject argObj, jfieldID jfieldId) {
    DL_INFO
    for(const auto &soname : filterSoList) {
        if (strstr(info.dli_fname, soname.c_str())) {
...
JNITRACE_HOOK_DEF(jbyte, GetByteField, JNIEnv *env, jobject argObj, jfieldID jfieldId) {
    DL_INFO
    for(const auto &soname : filterSoList) {
        if (strstr(info.dli_fname, soname.c_str())) {
...
JNITRACE_HOOK_DEF(jchar, GetCharField, JNIEnv *env, jobject argObj, jfieldID jfieldId) {
    DL_INFO
    for(const auto &soname : filterSoList) {
        if (strstr(info.dli_fname, soname.c_str())) {
...
JNITRACE_HOOK_DEF(jshort, GetShortField, JNIEnv *env, jobject argObj, jfieldID jfieldId) {
    DL_INFO
    for(const auto &soname : filterSoList) {
        if (strstr(info.dli_fname, soname.c_str())) {
...
JNITRACE_HOOK_DEF(jint, GetIntField, JNIEnv *env, jobject argObj, jfieldID jfieldId) {
    DL_INFO
    for(const auto &soname : filterSoList) {
        if (strstr(info.dli_fname, soname.c_str())) {
...
JNITRACE_HOOK_DEF(jlong, GetLongField, JNIEnv *env, jobject argObj, jfieldID jfieldId) {
    DL_INFO
    for(const auto &soname : filterSoList) {
        if (strstr(info.dli_fname, soname.c_str())) {
...
JNITRACE_HOOK_DEF(jfloat, GetFloatField, JNIEnv *env, jobject argObj, jfieldID jfieldId) {
    DL_INFO
    for(const auto &soname : filterSoList) {
        if (strstr(info.dli_fname, soname.c_str())) {
...
JNITRACE_HOOK_DEF(jdouble, GetDoubleField, JNIEnv *env, jobject argObj, jfieldID jfieldId) {
    DL_INFO
    for(const auto &soname : filterSoList) {
        if (strstr(info.dli_fname, soname.c_str())) {
...
JNITRACE_HOOK_DEF(jobject, GetStaticObjectField, JNIEnv *env, jclass clazz, jfieldID jfieldId) {
    DL_INFO
    for(const auto &soname : filterSoList) {
        if (strstr(info.dli_fname, soname.c_str())) {
        jobject ret = orig_GetStaticObjectField(env, clazz, jfieldId);
        if (ret != nullptr) {
            Jnitrace::getJObjectInfoInternal(env, ret, "GetStaticObjectField result object :", true,
                                             nullptr);
        }
        return ret;
    }
...
JNITRACE_HOOK_DEF(jboolean, GetStaticBooleanField, JNIEnv *env, jclass clazz, jfieldID jfieldId) {
    DL_INFO
    for(const auto &soname : filterSoList) {
        if (strstr(info.dli_fname, soname.c_str())) {
...
JNITRACE_HOOK_DEF(jbyte, GetStaticByteField, JNIEnv *env, jclass clazz, jfieldID jfieldId) {
    DL_INFO
    for(const auto &soname : filterSoList) {
        if (strstr(info.dli_fname, soname.c_str())) {
...
//            os << "GetStaticByteField result jbyte :  " << ret << "\n";;
        write(string("GetStaticByteField result jbyte : ").append(to_string(ret)).append("\n"));
        return ret;
    }
    return orig_GetStaticByteField(env, clazz, jfieldId);
}
//jchar       (*GetStaticCharField)(JNIEnv*, jclass, jfieldID);
JNITRACE_HOOK_DEF(jchar, GetStaticCharField, JNIEnv *env, jclass clazz, jfieldID jfieldId) {
    DL_INFO
    for(const auto &soname : filterSoList) {
        if (strstr(info.dli_fname, soname.c_str())) {
        ...
        write(string("GetStaticCharField result jchar : ").append(to_string(ret)).append("\n"));
        return ret;
    }
    return orig_GetStaticCharField(env, clazz, jfieldId);
}
//jshort      (*GetStaticShortField)(JNIEnv*, jclass, jfieldID);
JNITRACE_HOOK_DEF(jshort, GetStaticShortField, JNIEnv *env, jclass clazz, jfieldID jfieldId) {
    DL_INFO
    for(const auto &soname : filterSoList) {
        if (strstr(info.dli_fname, soname.c_str())) {
        ...
        write(string("GetStaticShortField result jshort : ").append(to_string(ret)).append("\n"));
        return ret;
    }
    return orig_GetStaticShortField(env, clazz, jfieldId);
}
//jint        (*GetStaticIntField)(JNIEnv*, jclass, jfieldID);
JNITRACE_HOOK_DEF(jint, GetStaticIntField, JNIEnv *env, jclass clazz, jfieldID jfieldId) {
    DL_INFO
    for(const auto &soname : filterSoList) {
        if (strstr(info.dli_fname, soname.c_str())) {
...
JNITRACE_HOOK_DEF(jlong, GetStaticLongField, JNIEnv *env, jclass clazz, jfieldID jfieldId) {
    DL_INFO
    for(const auto &soname : filterSoList) {
        if (strstr(info.dli_fname, soname.c_str())) {
        ...
        write(string("GetStaticLongField result jlong :  ").append(to_string(ret)).append("\n"));
        return ret;
    }
    return orig_GetStaticLongField(env, clazz, jfieldId);
}
//jfloat      (*GetStaticFloatField)(JNIEnv*, jclass, jfieldID);
JNITRACE_HOOK_DEF(jfloat, GetStaticFloatField, JNIEnv *env, jclass clazz, jfieldID jfieldId) {
    DL_INFO
    for(const auto &soname : filterSoList) {
        if (strstr(info.dli_fname, soname.c_str())) {
        ...
        write(string("GetStaticFloatField result jfloat :  ").append(to_string(ret)).append("\n"));
        return ret;
    }
    return orig_GetStaticFloatField(env, clazz, jfieldId);
}
//jdouble     (*GetStaticDoubleField)(JNIEnv*, jclass, jfieldID);
JNITRACE_HOOK_DEF(jdouble, GetStaticDoubleField, JNIEnv *env, jclass clazz, jfieldID jfieldId) {
    DL_INFO
    for(const auto &soname : filterSoList) {
        if (strstr(info.dli_fname, soname.c_str())) {
        ...
        write(string("GetStaticDoubleField result jdouble :  ").append(to_string(ret)).append(
                "\n"));
        return ret;
    }
